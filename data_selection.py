...
class Dataselect:
    ...
    @st.cache_data  # 데이터 가져오는 것에는 data
    def getgongsi(_self, date, flag, code):
        todate = str(date).replace("-", "")
        if flag == 1:
            sql = """
                EXEC [SL_GetInformation] ?,?,?,?,?
                """
            params = (todate, 14, "", "", code)
    ...
    @st.cache_data  # 데이터 가져오는 것에는 data
    def getstockreturnbymarketcap(_self, date, mktflag, options):
        todate = str(date).replace("-", "")
        # frdate = _self.getCalendar(todate, termflag, term)
        sql = "EXEC [SL_Getstockreturn] ?,?,?,?"
        params = ("", todate, mktflag, options)
        df = pd.read_sql(sql, con=_self.db_init, params=params)
        return df
    ...
    def select_term_and_flag(
        self,
        options=("1일", "1주", "1개월", "2개월", "3개월", "6개월"),
        default_index=5,
    ):
