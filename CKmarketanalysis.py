...
class Dataselect():
    def __init__(self, date,server, database, user_id, password,driver):
        self.server = server
    ...
    # @st.cache_resource
    def init_db(self):
        connection_string = f"mssql+pyodbc://{self.user_id}:{self.password}@{self.server}/{self.database}?{self.driver}"
...
class_data = Dataselect(date,st.secrets["server"],st.secrets["database"],st.secrets["username"],st.secrets["password"],st.secrets["driver"])
...
if date and add_selectbox=="관심종목":
    ...
    with st.expander('📑 관심종목 등록'):
        # 주식 목록 가져오기
        stock_list = class_data.getstockmater(date)
        stock_options = {f"{row['stockcode']} - {row['stockname']}": (row['stockcode'], row['stockname']) for index, row in stock_list.iterrows()}
        stock_choice = st.selectbox("Choose a stock", list(stock_options.keys()))  

        if st.button("Add to Interested Stocks"):
            
            stock_code, stock_name = stock_options[stock_choice]
            class_data.insert_interested_stock(interestname,stock_code, stock_name)
            st.success(f"{stock_name} added to interested stocks successfully!")
