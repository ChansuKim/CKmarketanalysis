...
MENU_OPTIONS = {
    ...
    "🎭테마분석": "theme_analysis",
    "📊주식분석": "stock_analysis",
    "💹옵션분석": "option_analysis",
    "🔖트레이딩전략": "trading_strategy",
    "📅시즈널리티": "seasonality",
    "💸Systemtrading(Live)": "system_trading",
}
...
def visualize_treemap(df, flag):
    ...
    if flag == 1:
        ...
        # 모든 값이 0보다 크거나 모든 값이 0보다 작은 경우를 처리
        if vmin >= 0:
            # 모든 값이 0보다 큰 경우
            norm = plt.Normalize(vmin, vmax)
            colors = plt.cm.Greens(norm(df["수익률(%)"]))
        elif vmax <= 0:
            # 모든 값이 0보다 작은 경우
            norm = plt.Normalize(vmin, vmax)
            colors = plt.cm.Reds_r(norm(df["수익률(%)"]))
        else:
            # 값들이 0을 기준으로 나뉘는 경우
        ...
        squarify.plot(
            sizes=df["평균 거래대금"],
            label=df["테마명"] + "(" + df["수익률(%)"].astype(str) + "%)",
            color=colors,
            alpha=0.8,
            ax=ax,
    ...
    if flag == 2:
        ...
        if vmin >= 0:
            # 모든 값이 0보다 큰 경우
            norm = plt.Normalize(vmin, vmax)
            colors = plt.cm.Greens(norm(df["ret"]))
        elif vmax <= 0:
            # 모든 값이 0보다 작은 경우
            norm = plt.Normalize(vmin, vmax)
            colors = plt.cm.Reds_r(norm(df["ret"]))
        else:
            # 값들이 0을 기준으로 나뉘는 경우
            norm = mcolors.TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)
            colors = plt.cm.RdYlGn(norm(df["ret"]))

        # norm = mcolors.TwoSlopeNorm(vmin=df["ret"].min(), vcenter=0, vmax=df["ret"].max())
        # colors = plt.cm.RdYlGn(norm(df["ret"]))
...
# @st.cache_data
def get_maxdate(class_data, todate):
...
def plot_backtest_single(class_data, date, flag, termflag, term, code, title):
...
def plot_backtest_multiple(class_data, date, flag, termflag, term, codes):
    fig = go.Figure()
...
def plot_backtest_multiple_dynamic(class_data, date, flag, termflag, term):
    fig = go.Figure()
...
def visualize_heatmap_seasonaliy(class_data, date, flag, termflag, term, title, code):
    ...
    if pivot_overnight.empty:
        st.warning("No data available to display for Returns.")
    else:
        # 히트맵 생성
...
def handle_system_trading(class_data, date):
    ...
    strategies = {
        "jongbe_1500": "Strategy 1",
        "jongbe_1502": "Strategy 2",
        "jongbe_1504": "Strategy 3",
        "jongbe_1458": "Strategy 4",
    }
    ...
    # 그래프 그리기
    df_st["logdate"] = pd.to_datetime(df_st["logdate"], format="%Y%m%d").dt.strftime(
        "%Y-%m-%d"
    )
...
def handle_dashboard(Main_Data, date):
    st.header("🌟DASH BOARD")
    os_date = Main_Data.getmaxdate(date, 4)
    os_date_bond = Main_Data.getmaxdate(date, 4)
    ...
    with st.container():
        ...
        for label, flag, code in markets:
            if label in ["KOSPI", "KOSDAQ"]:
                price, delta = Main_Data.getCurrentPrice(date, flag, code)
            else:
                price, delta = Main_Data.getCurrentPrice(os_date, flag, code)
    ...
    with st.container():
        with col1:
            st.markdown("**시장지수**")
            df = Main_Data.marketcondition(os_date, 2)
            st.dataframe(df, use_container_width=True, hide_index=True)
        with col2:
            st.markdown("**상품**")
            df = Main_Data.marketcondition(os_date_bond, 3)
            st.dataframe(df, use_container_width=True, hide_index=True)
    ...
    with st.container():
        with col3:
            st.markdown("**환율**")
            df = Main_Data.marketcondition(os_date, 4)
            st.dataframe(df, use_container_width=True, hide_index=True)
        with col4:
            st.markdown("**채권**")
            df = Main_Data.marketcondition(os_date_bond, 5)
            st.dataframe(df, use_container_width=True, hide_index=True)
    ...
    with st.container():
        ...
        with col5:
            st.markdown("**코스피**")
            df = Main_Data.getstockreturnbymarketcap(date, "kospi", "13")
            visualize_treemap(df, 2)
        with col6:
            st.markdown("**코스닥**")
            df = Main_Data.getstockreturnbymarketcap(date, "kosdaq", "13")
            visualize_treemap(df, 2)
        col7, col8 = st.columns(2)
        with col7:
            st.markdown("**코넥스**")
            df = Main_Data.getstockreturnbymarketcap(date, "konex", "13")
            visualize_treemap(df, 2)
        with col8:
            st.markdown("**K-OTC**")
            df = Main_Data.getstockreturnbymarketcap(date, "k-otc", "13")
            visualize_treemap(df, 2)

    tab1, tab2 = st.tabs(["🔝 상위", "🔻하위"])
    with tab1:
        st.markdown("**코스피 주간 Top 10**")
        df = Main_Data.marketcondition(date, 6)
        st.dataframe(df, use_container_width=True, hide_index=True)
    with tab2:
        st.markdown("**코스피 주간 Bottom 10**")
        df = Main_Data.marketcondition(date, 14)
        st.dataframe(df, use_container_width=True, hide_index=True)
    tab3, tab4 = st.tabs(["🔝 상위", "🔻하위"])
    with tab3:
        st.markdown("**코스닥 주간 Top 10**")
        df = Main_Data.marketcondition(date, 7)
        st.dataframe(df, use_container_width=True, hide_index=True)
    with tab4:
        st.markdown("**코스닥 주간 Bottom 10**")
        df = Main_Data.marketcondition(date, 15)
        st.dataframe(df, use_container_width=True, hide_index=True)
    tab9, tab10 = st.tabs(["🔝 상위", "🔻하위"])
    with tab9:
        st.markdown("**ETF 주간 Top 10**")
        df = Main_Data.marketcondition(date, 8)
        st.dataframe(df, use_container_width=True, hide_index=True)
    with tab10:
        st.markdown("**ETF 주간 Bottom 10**")
        df = Main_Data.marketcondition(date, 13)
        st.dataframe(df, use_container_width=True, hide_index=True)
    tab5, tab6 = st.tabs(["🔝 상위", "🔻하위"])
    with tab5:
        st.markdown("**코넥스 주간 Top 10**")
        df = Main_Data.marketcondition(date, 9)
        st.dataframe(df, use_container_width=True, hide_index=True)
    with tab6:
        st.markdown("**코넥스 주간 Bottm 10**")
        df = Main_Data.marketcondition(date, 16)
        st.dataframe(df, use_container_width=True, hide_index=True)
    tab7, tab8 = st.tabs(["🔝 상위", "🔻하위"])
    with tab7:
        st.markdown("**K-OTC 주간 Top 10**")
        df = Main_Data.marketcondition(date, 10)
        st.dataframe(df, use_container_width=True, hide_index=True)
    with tab8:
        st.markdown("**K-OTC 주간 Bottom 10**")
        df = Main_Data.marketcondition(date, 17)
        st.dataframe(df, use_container_width=True, hide_index=True)


# if date and add_selectbox == "🎭테마분석":
def handle_theme_analysis(class_data, date):
    st.header("📈테마수익률 현황")
    st.write("조회일 : ", date)
...
def handle_market_analysis(Main_Data, date):
    st.write("조회일 : ", date)
    st.header("📈 국내시장현황")
    ...
    with tab1:
        term, termflag = Main_Data.select_term_and_flag()
        ...
        with col6:
            ...
            df_price = Main_Data.getindexprice(date, "u201", "D", termflag, term)
            df_price["logdate"] = pd.to_datetime(df_price["logdate"])
            fig_m = Main_Data.create_candlestick_chart(
        ...
        with col11:
            df_price = Main_Data.getindexprice_sugup(date, "u001", "D", termflag, term)
            df_price["logdate"] = pd.to_datetime(
                df_price["logdate"]
            )  # Ensure datetime is in the correct format
            fig_d = px.line(
            ...
            st.plotly_chart(fig_d, use_container_width=True)
        with col12:
            df_price = Main_Data.getindexprice_sugup(date, "u201", "D", termflag, term)
            df_price["logdate"] = pd.to_datetime(
                df_price["logdate"]
            )  # Ensure datetime is in the correct format
            fig_d = px.line(
        ...
        with col13:
            df_price = Main_Data.getmarketinfo(date, termflag, term, 10)
            df_price["logdate"] = pd.to_datetime(df_price["logdate"])
            # 서브플롯 생성
            fig = make_subplots(
                rows=2,
        ...
        with col14:
            df_price = Main_Data.getmarketinfo(date, termflag, term, 11)
            df_price["logdate"] = pd.to_datetime(df_price["logdate"])
            fig = make_subplots(
                rows=4,
                cols=1,
    ...
    with tab2:
        col1, col2 = st.columns(2)
        with col1:
            df_price = Main_Data.getindexprice(date, "U001", "M", None, None)
            df_price["datetime"] = pd.to_datetime(df_price["datetime"])
            fig_m = Main_Data.create_candlestick_chart(
            ...
            st.plotly_chart(fig_m, use_container_width=True)
        with col2:
            df_price = Main_Data.getindexprice(date, "U201", "M", None, None)
            df_price["datetime"] = pd.to_datetime(df_price["datetime"])
            fig_m = Main_Data.create_candlestick_chart(
                df_price, "Intraday KOSDAQ Candestick Chart", "date", "price"
            )
        ...
        col3, col4 = st.columns(2)
        with col3:
            df_price = Main_Data.getindexprice_sugup(date, "u001", "M", None, None)
            ...
            st.plotly_chart(fig_d, use_container_width=True)
        with col4:
            df_price = Main_Data.getindexprice_sugup(date, "u201", "M", None, None)
...
def handle_stock_analysis(Main_Data, date):
    ...
    col1, col2 = st.columns(2)
    with col1:
        stock_conditions = Main_Data.getconditionlist()
    ...
    try:
        ...
        stock_list = Main_Data.getstocklistbycondition(date, condition_choice)
        stockcodes = "".join([f"{code}|" for code in stock_list["stockcode"]])
        stockcodes = stockcodes.rstrip("|")
        searchdate = date

        df = Main_Data.getthemestock(searchdate, stockcodes, 2)
        st.dataframe(df, use_container_width=True, hide_index=True)
        selected_stock = None
        col3, col4 = st.columns(2)
        with col3:
        ...
        df_aftermarket = Main_Data.getAftermarketprice(searchdate, selected_stock, 4)
        with col4:
        ...
        try:
            with col111:
                if st.button("Previous Day"):
                    st.session_state["chart_date"] = Main_Data.getdatediff(
        ...
        col7, col8 = st.columns(2)
        with col7:
            df_price = Main_Data.getstockprice(chartdate, selected_stock, "M")
        ...
        if condition_choice == "유상증자":
            ...
            with st.expander("공시내용", expanded=False):
                ...
                st.markdown(html_table, unsafe_allow_html=True)
        df_lastnews = Main_Data.getLastnews(selected_stock)
        df_gongsi = Main_Data.getstockgongsi(date, selected_stock)
        df_naverdiscussion = Main_Data.getNaverdiscussion(selected_stock)
        col4, col5 = st.columns(2)
        with col4:
            st.markdown(
                """
...
def handle_seasonality(Main_Data, date):
    st.header("📈시즈널리티")
    st.write("조회일 : ", date)
    term, termflag = Main_Data.select_term_and_flag(
        options=(
...
def handle_trading_strategy(Main_Data, date):
    st.header("📈트레이딩 전략")
    st.write("조회일 : ", date)
    term, termflag = Main_Data.select_term_and_flag(
        options=(
...
def handle_option_analysis(Main_Data, date):
    st.write("조회일 : ", date)
    term, termflag = Main_Data.select_term_and_flag(default_index=2)
    st.header("📈옵션 현황")
    col5, col6 = st.columns(2)
    with col5:
    ...
    with tab1:
        ...
        col3, col4 = st.columns(2)
        with col3:
            df_option = Main_Data.getoptionprice(date, "d", 0, "c", termflag, term)
            # Generate and display charts for Call options
            for metric, desc in option_metrics.items():
                fig = Main_Data.create_line_chart(
                    df_option, f"Call {desc}", "logdate", metric
                )
                st.plotly_chart(fig, use_container_width=True)

        with col4:
            df_option = Main_Data.getoptionprice(date, "d", 0, "p", termflag, term)
            # Generate and display charts for Call options
            for metric, desc in option_metrics.items():
                fig = Main_Data.create_line_chart(
    ...
    with tab2:
        col1, col2 = st.columns(2)
        with col1:
            df_option = Main_Data.getoptionprice(date, "m", 0, "c", None, None)
            # Generate and display charts for Call options
            for metric, desc in option_metrics.items():
                fig = Main_Data.create_line_chart(
        ...
        with col2:
            df_option = Main_Data.getoptionprice(date, "m", 0, "p", None, None)
            # Generate and display charts for Call options
            for metric, desc in option_metrics.items():
                fig = Main_Data.create_line_chart(
...
def setup_main_ui():
    ...
    selected_menu = st.selectbox(
        "🔍 찾고 싶은 정보를 선택하세요.", list(MENU_OPTIONS.keys())
    )
    date = st.date_input("📅 조회 시작일을 선택해 주세요", max_value=datetime.today())

    return selected_menu, date


def setup_sidebar(class_data):
...
if __name__ == "__main__":
    selected_menu, date = setup_main_ui()
    Main_Data = Dataselect(
        date,
        st.secrets["server"],
        st.secrets["database"],
        st.secrets["username"],
    ...
    db_connection = Main_Data.init_db()
    todate = str(date).replace("-", "")
    date = get_maxdate(Main_Data, todate)

    st.divider()

    with st.sidebar:
        setup_sidebar(Main_Data)

    # 메뉴 처리
    # 메뉴 처리 시 class_data와 date 순서 변경
    menu_handlers = {
        "system_trading": lambda date, Main_Data: handle_system_trading(
            Main_Data, date
        ),
        "dashboard": lambda date, Main_Data: handle_dashboard(Main_Data, date),
        "market_analysis": lambda date, Main_Data: handle_market_analysis(
            Main_Data, date
        ),
        "stock_analysis": lambda date, Main_Data: handle_stock_analysis(
            Main_Data, date
        ),
        "option_analysis": lambda date, Main_Data: handle_option_analysis(
            Main_Data, date
        ),
        "trading_strategy": lambda date, Main_Data: handle_trading_strategy(
            Main_Data, date
        ),
        "seasonality": lambda date, Main_Data: handle_seasonality(Main_Data, date),
        "theme_analysis": lambda date, Main_Data: handle_theme_analysis(
            Main_Data, date
        ),
    }

    selected_handler = menu_handlers.get(MENU_OPTIONS[selected_menu])
    if selected_handler:
        selected_handler(date, Main_Data)
